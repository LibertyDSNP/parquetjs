...
const bloomFilterReader = require('./bloomFilterReader');
...
class ParquetReader {
  ...
  async getBloomFiltersFor(columns) {
    const columnChunks = this.envelopeReader.getAllColumnChunksFor(columns);
    const offsetsByColumn = siftAllOffsets(columnChunks);

    return await readFilterBlocksFrom(offsetsByColumn, this.envelopeReader)
  }
...
class ParquetEnvelopeReader {
  ...
  getAllColumnChunksFor(paths, row_groups) {
    if (!row_groups) {
      row_groups = this.metadata.row_groups;
    }

    return row_groups.map((rowGroup, index) =>
              paths.map(column => ({
                  rowGroup: index,
                  column: this.getColumn(column, rowGroup)
              }))
            )
            .flat()
  }
