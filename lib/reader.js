...
class ParquetReader {
  ...
  async getBloomFilters(columnNames) {
    const bloomFilterOffsets =
      parseBloomFilterOffsets(getBloomFilterColumnChunks(columnNames, this.metadata, this.envelopeReader));

    const names = Object.keys(bloomFilterOffsets);
    return await Promise.all(names.map(async (name) => {
      const columnOffsets = bloomFilterOffsets[name]
      const filters = await getBloomFilterForOffsets(columnOffsets, this.envelopeReader)
