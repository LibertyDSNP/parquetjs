...
class ParquetReader {
  ...
  static async openEnvelopeReader(envelopeReader, opts) {
    ...
    if (options.metadata) {
      return new ParquetReader(opts.metadata, envelopeReader, options);
    }
    try {
      await envelopeReader.readHeader();
      let metadata = await envelopeReader.readFooter();
      return new ParquetReader(metadata, envelopeReader, options);
    } catch (err) {
   ...
   * instead
   */
  constructor(metadata, envelopeReader, options) {
    ...
    if (this.metadata.row_groups && !this.metadata.json && !realOptions.rawStatistics) {
