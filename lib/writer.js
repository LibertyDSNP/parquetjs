...
class ParquetEnvelopeWriter {
  ...
  // 2. update metadata for rowGroup
  // 
  writeBloomFilters(_rowGroups) {
    ...
    rowGroups.forEach(group => {
      group.columns.forEach(column => {
        ...
        const filterBlocks = this.bloomFilters[columnName].splitBlockFilter;

        const serializedBloomFilterData = bloomFilterWriter.serializeFilterData(filterBlocks);
