...
const bloomFilterWriter = require("./bloomFilterWriter")
...
class ParquetEnvelopeWriter {
  ...
  constructor(schema, writeFn, closeFn, fileOffset, opts) {
    ...
    (opts.bloomFilters || []).forEach(bloomOption => {
      this.bloomFilters[bloomOption.column] = bloomFilterWriter.buildFilterBlocks(bloomOption)
    });
  }
  ...
  writeBloomFilters(_rowGroups) {
    ...
    rowGroups.forEach(group => {
      group.columns.forEach(column => {
        ...
        const splitBlockBloomFilter = this.bloomFilters[columnName]
        const filterBlocks = splitBlockBloomFilter.splitBlockFilter;
        const filterByteSize = splitBlockBloomFilter.optNumFilterBytes();

        const serializedBloomFilterData = bloomFilterWriter.serializeFilterData({filterBlocks, filterByteSize});
...
function encodePages(schema, rowBuffer, opts) {
  ...
  for (let field of schema.fieldList) {
    ...
    if (opts.bloomFilters != undefined && (field.name in opts.bloomFilters)) {
      const splitBlockBloomFilter = opts.bloomFilters[field.name];
      for (const value of values.values) {
        splitBlockBloomFilter.insert(value);
      }
    }
