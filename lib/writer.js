...
class ParquetWriter {
  ...
  async appendRow(row) {
    ...
    const fil = new sbbf.default().init()
  }
  ...
  writeBloomFilters(_rowGroups) {
    ...
    this.schema.fieldList.forEach( (c,i) => {
      rowGroups.forEach(group => {
        ...
        const blocks = this.bloomFilters[columnName];

        const bloomFilterHeader = new parquet_thrift.BloomFilterHeader();
        bloomFilterHeader.numBytes = blocks.length;; // option data;
        ...
        const serializedBloomHeaders = parquet_util.serializeThrift(bloomFilterHeader);

        column.meta_data.bloom_filter_offset = this.offset;

        const concatenatedBlocks = Buffer.concat(blocks.map(block => Buffer.from(block.buffer)));
        const buffer = Buffer.concat([serializedBloomHeaders, concatenatedBlocks]);

        this.writeSection(buffer);
      })
    })
  }
