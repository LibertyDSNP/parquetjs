...
const buildFilterBlocks = ({
  ...
  if (numFilterBytes)
    splitblockBloomFilter.setOptionNumFilterBytes(numFilterBytes).init();

  if (falsePositiveRate)
    splitblockBloomFilter.setOptionFalsePositiveRate(falsePositiveRate);
...
const serializeFilterData = ({ filterBlocks, filterByteSize }) => {
  ...
  const serilizedFilterHeaders = serializeFilterHeaders(filterByteSize);
