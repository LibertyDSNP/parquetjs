 ...
 */
const fromJsonSchemaArray = (
  fieldValue: SupportedJSONSchema4,
  optionalFieldList: boolean
): FieldDefinition => {
  ...
  switch (fieldValue.items.type) {
    ...
    case "object":
      return fields.createStructListField(
        fromJsonSchema(fieldValue.items),
        optionalFieldList
      )
    default:
      throw new UnsupportedJsonSchemaError(
        `Array field type ${JSON.stringify(fieldValue.items)} is unsupported.`
      )
  }
}
...
const fromJsonSchemaField =
  ...
  (fieldName: string, fieldValue: JSONSchema4): FieldDefinition => {
    if (!isJsonSchemaSupported(fieldValue)) {
      throw new UnsupportedJsonSchemaError(
        `Field: ${fieldName} has an unsupported schema`
      )
    }
